...
use crate::turtle_kinds_src::TURTLE_KINDS_SRC;
...
impl FromStr for LanguageKind {
    ...
    fn from_str(kind: &str) -> Result<Self, Self::Err> {
        match kind {
            ...
            _ => Err(format!(
                "Language {kind} not supported, please use: `js`, `css`, `json`, `grit`, `graphql`, `html`, `yaml`, `markdown`, `tailwind`, or `turtle`"
            )),
        }
    }
}
...
impl LanguageKind {
    define_language_kind_functions!([Js, Css, Json, Graphql, Grit, Html, Yaml, Markdown, Tailwind, Turtle]);
    ...
    pub fn load_grammar(&self) -> &'static str {
        match self {
            ...
            Self::Turtle => include_str!("../turtle.ungram"),
        }
    }
