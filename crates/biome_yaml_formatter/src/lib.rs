...
use biome_formatter::{
    CstFormatContext, FormatContext, FormatLanguage, FormatOwnedWithRule,
    FormatRefWithRule, FormatState, TransformSourceMap, VecBuffer, write,
};
use biome_formatter::format_element::document::Document;
use biome_formatter::{Formatted, Printed};
use biome_yaml_syntax::{
    AnyYamlBlockNode, YamlLanguage, YamlSyntaxNode, YamlSyntaxToken,
};
...
impl FormatLanguage for YamlFormatLanguage {
    ...
    fn is_range_formatting_node(&self, node: &SyntaxNode<Self::SyntaxLanguage>) -> bool {
        AnyYamlBlockNode::can_cast(node.kind())
    }
...
///
/// This is a custom implementation because YAML uses zero-width synthetic tokens
/// (MAPPING_START, MAPPING_END, FLOW_START, FLOW_END) that share start positions
/// with real tokens. The standard token tracker panics on duplicate offsets, so
/// we disable token tracking for YAML formatting.
pub fn format_node(
    options: YamlFormatOptions,
    root: &YamlSyntaxNode,
) -> FormatResult<Formatted<YamlFormatContext>> {
    let language = YamlFormatLanguage::new(options);
    let context = language.create_context(root, None, false);
    let format_node = FormatRefWithRule::new(root, FormatYamlSyntaxNode);

    let mut state = FormatState::new(context);
    state.set_token_tracking_disabled(true);
