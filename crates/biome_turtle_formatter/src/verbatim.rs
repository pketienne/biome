use crate::context::TurtleFormatContext;
...
pub struct FormatTurtleVerbatimNode<'node> {
    node: &'node TurtleSyntaxNode,
    kind: VerbatimKind,
    format_comments: bool,
}

impl Format<TurtleFormatContext> for FormatTurtleVerbatimNode<'_> {
...
/// Formats bogus nodes.
pub fn format_bogus_node(node: &TurtleSyntaxNode) -> FormatTurtleVerbatimNode<'_> {
    FormatTurtleVerbatimNode {
        node,
...
pub fn format_suppressed_node(node: &TurtleSyntaxNode) -> FormatTurtleVerbatimNode<'_> {
    FormatTurtleVerbatimNode {
        node,
...
impl<F, Item> Format<TurtleFormatContext> for FormatNodeOrVerbatim<F>
where
    F: FormatWithRule<TurtleFormatContext, Item = Item>,
    Item: AstNode<Language = TurtleLanguage>,
{
    fn fmt(&self, f: &mut Formatter<TurtleFormatContext>) -> FormatResult<()> {
