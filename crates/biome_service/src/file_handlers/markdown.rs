...
use biome_configuration::markdown::{
    MarkdownAssistConfiguration, MarkdownAssistEnabled, MarkdownFormatterConfiguration,
    MarkdownFormatterEnabled, MarkdownLinterConfiguration, MarkdownLinterEnabled,
};
...
impl From<MarkdownFormatterConfiguration> for MarkdownFormatterSettings {
    fn from(config: MarkdownFormatterConfiguration) -> Self {
        Self {
...
impl From<MarkdownLinterConfiguration> for MarkdownLinterSettings {
    fn from(configuration: MarkdownLinterConfiguration) -> Self {
        Self {
...
impl From<MarkdownAssistConfiguration> for MarkdownAssistSettings {
    fn from(configuration: MarkdownAssistConfiguration) -> Self {
        Self {
            enabled: configuration.enabled,
        }
    }
...
pub use biome_markdown_formatter::context::MarkdownFormatOptions;
...
fn format(
    ...
    let options = settings.format_options::<MarkdownLanguage>(path, document_file_source);
...
fn lint(params: LintParams) -> LintResults {
    ...
    let analyzer_options = workspace_settings.analyzer_options::<MarkdownLanguage>(
        params.path,
        &params.language,
...
pub(crate) fn fix_all(params: FixAllParams) -> Result<FixFileResult, WorkspaceError> {
    let mut tree: MdDocument = params.parse.tree();
    ...
    loop {
        ...
        let result = process_fix_all.process_action(action, |root| {
            tree = match MdDocument::cast(root) {
                Some(tree) => tree,
                None => return None,
            };
